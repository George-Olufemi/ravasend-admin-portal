...
import { BarChart2, ChevronRight, Bell } from "lucide-react";
...
function TopBar({ title }: { title: string }) {
  ...
  return (
    <div className="h-[52px] border-b border-border/50 flex items-center px-7 gap-3 shrink-0" style={{ background: "#07051A" }}>
      <div className="size-6 rounded-md bg-primary/20 flex items-center justify-center">
        <BarChart2 size={13} className="text-primary" />
      </div>
      <span className="text-[12px] text-muted-foreground font-medium">Admin Dashboard</span>
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        <div className="size-8 rounded-full flex items-center justify-center text-[10px] font-bold text-white" style={{ background: "linear-gradient(135deg, #7B3FE4, #5B2AB8)" }}>{userInitials}</div>
      </div>
    </div>
  );
}
...
const AdminLayout = () => {
  ...
  return (
    <div className="min-h-screen flex w-full text-foreground" style={{ background: "#07051A" }}>
      <AdminSidebar withdrawalPaused={withdrawalPaused} />
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <TopBar title={title} />
        <main className="flex-1 overflow-y-auto p-7" style={{ background: "#07051A" }}>
